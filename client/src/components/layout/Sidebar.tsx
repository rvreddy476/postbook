...
import { Home, Film, Tv, MessageSquare, User, Bell, Settings, LogOut } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-[calc(100vh-4rem)] sticky top-16 border-r border-border bg-white px-4 py-6 z-40">
      <Link href="/profile" className="flex items-center gap-3 p-3 mb-6 rounded-xl hover:bg-muted transition-all cursor-pointer group" data-testid="link-profile">
        <Avatar className="h-12 w-12 border-2 border-border group-hover:border-foreground/30 transition-all">
            <AvatarFallback className="bg-foreground text-white font-semibold">{userInitials}</AvatarFallback>
        </Avatar>
        <div className="flex flex-col">
            <span className="font-heading font-bold text-base text-foreground" data-testid="text-username">{displayName}</span>
